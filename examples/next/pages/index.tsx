...
function Page() {
	...
	const handleMint = async () => {
		...
		const indexRouter = new IndexRouter(account, '0xB2779aD3265100C88645dec3422E1bc9d8Ee84A8')

		const autoRouter = new AutoRouter(indexRouter, zeroEx);

		if (selectedToken === 'USDC') {
			const usdc = new Erc20(account, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48');

			const value = BigNumber.from(utils.parseUnits(inputValue, await usdc.decimals()));

			const permit = await signERC2612Permit(window.ethereum, usdc.address, await account.address(), indexRouter.address, value.toString());

			setTx(await autoRouter.autoBuy(
					new Index(account, '0x632806bf5c8f062932dd121244c9fbe7becb8b48'),
					value,
					usdc,
					{
						deadline: permit.deadline, r: permit.r, s: permit.s, v: permit.v,
					}
				)
			)
	...
	return (
		<div>
			...
			<Input type='number' inputMode='decimal' value={inputValue}
