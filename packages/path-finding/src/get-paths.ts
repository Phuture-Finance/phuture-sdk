...
export interface PathInfo {
    path: Array<string>
    pairInfo: Array<PairInfo>
}

export const getPaths = (
    allPairsWithReserves: Array<PairInfo>,
    tokenIn: string,
    tokenOut: string,
    quotes: Array<string>,
): Array<PathInfo> => {
    const pathInfos: PathInfo[] = [...quotes].map<PathInfo>(() => ({path: [], pairInfo: []}))

    for (const info of allPairsWithReserves) {
        const containsTokenIn =
            info.token0 === tokenIn.toLowerCase() || info.token1 === tokenIn.toLowerCase()
        const containsTokenOut =
            info.token0 === tokenOut.toLowerCase() || info.token1 === tokenOut.toLowerCase()
        if (containsTokenIn && containsTokenOut) {
            ...
            const isToken0Quote =
                info.token0 !== tokenIn.toLowerCase() && info.token0 !== tokenOut.toLowerCase()
    ...
    return pathInfos.filter(
        ({path}) =>
            path[0] === tokenIn.toLowerCase() && path[path.length - 1] === tokenOut.toLowerCase(),
    )
}
