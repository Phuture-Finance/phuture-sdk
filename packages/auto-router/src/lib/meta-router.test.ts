import {MetaRouter, ProductType} from "./meta-router";
...
describe('MetaRouter', () => {
	...
	beforeAll(async ()=> {
		...
		savingsVaultAddresses = ['0xc365c3315cf926351ccaf13fa7d19c8c4058c8e1', '0x28c6c06298d514db089934071355e5743bf21d60']
		indexAddresses = ['0xf6562d37465dc23d52a8c19d45a9a7afd85fe1ec', '0x8167da7fa6554f2053ee92f77fd3d729323e0aca']
	...
	it('should be able to set product addresses', async () => {
		expect(metaRouter.getProductAddresses(ProductType.INDEX)).to.deep.eq(indexAddresses);
		expect(metaRouter.getProductAddresses(ProductType.SAVINGS_VAULT)).to.deep.eq(savingsVaultAddresses);
		...
		for(const address of savingsVaultAddresses) {
			expect(metaRouter.findProductType(address)).to.eq(ProductType.SAVINGS_VAULT)
		}
	})

	it('should route selectBuy to SavingsVault', async () => {
		const sharesAmount = BigNumber.from(10);
		savingsVaultInterface.contract.previewDeposit.mockReturnValueOnce(Promise.resolve(sharesAmount))
	...
	it('should route selectSell to SavingsVault', async () => {
		const sharesAmount = BigNumber.from(10);
		savingsVaultInterface.contract.previewRedeem.mockReturnValueOnce(Promise.resolve(sharesAmount))
		const result = await metaRouter.selectSell(savingsVaultInterface, sharesAmount);
		expect(result).to.deep.equal({
			isBurn: true,
