...
import {SavingsVault as SavingsVaultContractInterface, SavingsVaultViews as SavingsVaultViewsContractInterface} from '../types';
...
describe('SavingsVault', () => {
	...
	describe('SavingsVault constructed', () => {
		...
		beforeAll(async () => {
			...
			const savingsVaultViewsContract = new Mock<SavingsVaultViewsContractInterface>()
				.setup((c) => c.address)
			...
			savingsVaultViews = new SavingsVaultViews(account, savingsVaultViewsContract)
			savingsVault = new SavingsVault(account, savingsVaultContract, undefined, savingsVaultViews)
		});
		...
		it('SavingsVault should return formatted balance', async () => {
			const string_ = await savingsVault.formattedBalanceOf(constants.AddressZero);
			expect(string_).to.be.eq('1.23456789');
		});

		it('SavingsVaultViews should return apy', async () => {
			const apy = await savingsVaultViews.contract.getAPY(constants.AddressZero);
